...
class neo4j_broker:
    ...
    def node_num(self):
        num = self.graph.run("MATCH (n) RETURN COUNT(n) as num").data()
    ...
    def is_boundary_book_by_bid(self, bid):
        cur = self.graph.run('MATCH (b:Book) WHERE b.bid="{0}" AND NOT (b)-[:读者可能喜欢]->(:Book) RETURN b;'.format(bid))
        if not cur.data():
            return False
        else:
            return True
    ...
    def create_books_rel(self, bid1, bid2):
        query = "MATCH (B1:Book {{bid:'{0}'}}), (B2:Book {{bid:'{1}'}}) MERGE (B1)-[:读者可能喜欢]->(B2)".format(bid1,bid2)
        self.graph.run(query)
    ...
    def get_boundary_books_id(self):
        boundary_books_id = []
        cur = self.graph.run("MATCH (b:Book) WHERE NOT (b)-[:读者可能喜欢]->(:Book) RETURN b")
        if cur is not None:
            for node in cur:
                bid = node.data()['b']['bid']
